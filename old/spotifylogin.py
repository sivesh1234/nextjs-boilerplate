...
async def callback(request: Request):
    ...
    lineup_df = pd.read_csv('lineup.csv')
    ...
    # Save the dataframe to a CSV for debugging
    # matched_df.to_csv('adjusted_matched_lineup.csv', index=False)

    # Generate a list of unique stages
    stages = matched_df['Stage'].unique().tolist()

    # Render the HTML template for the playlist preview and schedule
